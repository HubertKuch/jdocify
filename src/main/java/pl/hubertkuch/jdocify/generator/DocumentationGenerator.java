...
public class DocumentationGenerator {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (packageToScan == null || packageToScan.isEmpty()) {
            ...
            log.error("Please configure the 'jdocify.scanPackage' system property in your build configuration.");
            System.exit(1);
        }
    ...
    public void generate(Set<Class<?>> classes) throws IOException {
        for (var clazz : classes) {
            ...
            var descriptionStrategies = List.of(
            ...
            var renderedTemplate = templateEngine.render(templateEngine.getTemplate("class.md.template"), classData);
            documentationWriter.write(clazz.getSimpleName(), renderedTemplate);
        }
    }

    private Map<String, String> processClass(Class<?> clazz, JavaDocParser javaDocParser, List<DescriptionStrategy> descriptionStrategies) throws IOException {
        ...
        data.put("class.name", documentedAnnotation.name().isEmpty() ? clazz.getSimpleName() : documentedAnnotation.name());
    ...
    private String processConstructors(Class<?> clazz, JavaDocParser javaDocParser) throws IOException {
    ...
    private String processMethods(Class<?> clazz, JavaDocParser javaDocParser, List<DescriptionStrategy> descriptionStrategies) throws IOException {
        ...
        for (var method : clazz.getDeclaredMethods()) {
            ...
            var description = descriptionStrategies.stream()
    ...
    private String getFilePath(Class<?> clazz) {
        return "src" + File.separator + "main" + File.separator + "java" + File.separator + clazz.getName().replace(".", File.separator) + ".java";
    }
}
